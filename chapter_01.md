...
かつてプログラマは、書いたコード行数によって評価されることがよくありました（source lines of code または SLOC で検索してみてください）。
このメトリクス（評価基準値）がどうなるかは明らかです。プログラミングを、画一的に訓練を受けた労働者が規格品を組み立てるライン作業のようなものと見なしている上司は、
個々のプログラマの生産性などというものは、定量的な数値で評価できると簡単に信じることができるでしょう。プログラマの良し悪しを比較し、ソフトウェア
を評価しようとや躍起になっている管理職たちにすれば、SLOC はあきらかに問題があるにも関わらず、何もないよりは遥かに使えるものと映るのです。
彼らにとっては、少なくとも「何か」を評価できる再利用可能なものさしなのです。

明らかに、このメトリクスはプログラマが開発したものではありません。SLOC は個人の努力とアプリケーションの複雑さについて、ある尺度を与えてはくれますが、
全体の品質については沈黙しています。それは、有能なプログラマを罰する一方、冗長を奨励し、ベースアプリケーションを損傷してしまうような、
エキスパートたちのゲームと化してしまいます。もしあなたの隣に座っている新人プログラマが、あなたなら数行で実装できるフィーチャを大量コードの書いて
実装してがゆえに生産性が高いと評価されているとしてら、あなたはどう思うでしょうか？このメトリクスでは、品質を毀損してしまっても評価される基準になってしまいます。
