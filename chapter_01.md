...
良くも悪くもできないのです。アジャイルのこうした考え方が正しいならば、もう２つのことも同様に正しいはずです。ひとつは、BUFD（Big Up Front Design）を実践に
することにはまったく意味がない、ということ（なぜならまったく正しくないからです）。そしてもうひとつは、アプリケーションがいつ完成するのかは誰も予見できない、
ということです。
...
支配感は心地いいものでしょうが、アプリケーションを実際に書くという行為の前ではかなく消え去ってしまうものです。

BUFD は、顧客とプログラマとの関係をどうしても敵対的なものに仕向けてしまいます。ソフトウェアが実査に動き出す前に作られたビッグ・デザインは、
そもそも正確ではないし、そういったある保証のもとでアプリケーションを書いているのでは、顧客のニーズにマッチするものにはならないでしょう。
顧客は、ソフトウェアを使ったときにはじめてそのことに気づきます。顧客は変更を求めます。プログラマは変更を固辞します。プログラマには別のスケジュールがあって、
それもすでに遅れ気味なのです。こうしてプロジェクトはだんだんと、関係者が成功させようとするものから、失敗を責められまいと凌ぐべきものへと変わっていくのです。
