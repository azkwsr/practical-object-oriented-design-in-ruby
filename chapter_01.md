...
オブジェクト指向なアプリケーションは、オブジェクトたちの間でやりとりされるメッセージからできています。メッセージはオブジェクトよりものちのち重要になってくるのですが、ここでは（それと最初のいくつかの章では）オブジェクトとメッセージを同等に扱うことにします。

#### 手続き型言語

オブジェクト指向プログラミングは、オブジェクト指向ではないプログラミング、たとえば「手続型」プログラミングと比べて「オブジェクト指向的」です。オブジェクト指向プログラミングと手続き型プログラミングについてそれぞれのスタイルの相違を考えることは有益なことです。何か一般的な手続き型プログラミング言語を想像して、それでシンプルなスクリプトを記述してください。変数を定義したら、ちょっとしたデータを割り当ててください。データを割り当てたら、変数を参照すればデータにアクセスできるようになります。

すべての手続き型言語と同じように、この言語にも異なる種類のデータから構成された小さくて固定された集合があります。それは、文字列、数値、配列、ファイルなどから構成されています。これらの異なる種類のデータは「データ型」と呼ばれます。それぞれのデータ型は、非常に限定されたものごとの種類を表しています。「文字列」型データは「ファイル」型データとは異なります。言語のシンタックスにはあらかじめデータ型に対する操作（オペレーション）が組み込まれており、いろいろなデータ型に対して可能な操作を行うことができます。例えば、文字列を連結したりファイルを読み込んだりすることなどです。

変数をつくったのは「あなた」で、あなたはそれぞれの変数に何が割り当てられているのかを知っています。利用可能な操作についてのあなたの見込みは、変数のデータ型についてのあなたの知識にもとづいています。文字列連結、数値計算、配列インデックス、ファイル読み込み、といったことが可能なことをあなたはあらかじめ知っています。

データ型とそれに対する操作はあらかじめ決まっています。それらは言語のシンタックスに組み込まれています。この手続き型言語では関数（あらかじめ定義されているいくつかの操作を、新しい名前をつけてひとつにまとめること）を作ることができるかもしれませんし、あるいは複雑なデータ構造（あらかじめ定義されているいくつかのデータ型を名前のついた配列に入れること）を定義できるかもしれません。ですが、あなたはまったく新しい操作を作ったり新しいデータ型を定義することはできません。あなたに見えるものがあなたに与えられたすべてです。

この言語では、あらゆる手続き型言語がそうであるように `data`（データ）と `behavior`（振る舞い）との間に隔絶が存在します。データはデータであり、振る舞いはデータとは異なるまったく別の何かです。データは変数に格納され振る舞いを通ると、振る舞いはデータに対してあれこれします。データは子供で、振る舞いという親がデータを毎朝学校に送り出すようなものです。振る舞いにとって、データが視界の外にある間は現実に何が起こっているかを知るすべがありません。データに何が起こるかは予測しがたく、広範囲に追跡することはできません。


#### オブジェクト指向言語

では次に、もうひとつ別の種類のプログラミング言語を想像してみましょう。それは Ruby のような、クラスをベースとしたオブジェクト指向型の言語です。データと振る舞いを分離して never the twain shall meet な（相互に相容れない）関係にしてしまうかわりに、Ruby はデータと振る舞いをひとつのオブジェクトに融合します。オブジェクトには振る舞いとデータがあり（データがない場合もありますが）、データへのアクセスは、そのデータを所有するオブジェクトだけがコントロールできます。オブジェクトたちはお互いにメッセージをやり取りすることで別のオブジェクトの振る舞いを喚起します。

Ruby には文字列型データのかわりに、文字列（String）オブジェクトがひとつ存在します。文字列を扱う操作は、プログラミング言語のシンタックスではなく文字列オブジェクトに組み込まれています。文字列オブジェクトはそれぞれに異なる文字列データを持つという点でお互いに異なりますが、お互いが同じように振る舞うという点で共通しています。文字列オブジェクトは自分のデータを世界から「カプセル化」つまり隠蔽します。オブジェクトは自分のデータをどの程度公開するのかをみずから決定するのです。

String オブジェクトは自分に対する可能な操作をみずから提供するので、Ruby は文字列型のデータについて特に何か知っている必要はありません。メッセージを送るための一般的な方法を提供すればそれでよいのです。文字列が `contcat` というメッセージを理解するなら、文字列を連結するためのシンタックスをプログラミング言語のレベルで Ruby が持つ必要はありません。Ruby はあるオブジェクトが別のオブジェクトに対して `concat` を送る方法を提供するだけでよいのです。

どんなに単純なアプリケーションであっても、ひとつの文字列、数値、ファイル、配列だけで済むようなことはないでしょう。ときに、単一の小さなオブジェクトひとつで済むようなことがあるとしても、実際には、同一の振る舞いを持ち、異なるデータをカプセル化する複数のオブジェクトを作りたいという状況のほうがはるかに一般的でしょう。

Ruby に代表されるクラスベースのオブジェクト指向言語では、`class`（クラス）を定義することで似たようなオブジェクトを生成するための設計図（blueprint）を書くことができます。クラスには `methods` （メソッド：振る舞いの定義です）と `attributes`（属性：変数の定義）が定義されます。メソッドはメッセージへの応答として呼び出されます。同じ名称のメソッドを複数の異なるオブジェクトで定義できます。Ruby の仕事は、そこでどのようなメッセージに対しても、適切なオブジェクトにあるメソッドを正確に見つけて呼び出すことです。

一度 *String*（文字列）クラスが存在してしまえば、`instantiate`（インスタンス化）あるいは String オブジェクトの新しい `instance` を作るためにそれを繰り返し利用できます。新たにインスタンス化されたそれぞれの *String* は、同一のメソッドを `implement`（実装）し、同一の属性名を利用しますが、そのデータを個別に異なります。それぞれのインスタンスは同一のメソッドを共有するのですべて *Strings* のように振る舞いますが、データはインスタンス間で異なるでそれぞれで異なる String を表しています。

*String* クラスは、単なる `data`（データ）以上の型（type）を定義します。あるオブジェクトの型を知れば、そのオブジェクトの振る舞いを推測できるようになります。ある手続き型の言語では、変数はひとつのデータ型に属していました。そしてこのデータ型を知れば、どのような操作がその変数に可能なのか推測できました。Ruby では、ひとつのオブジェクトが複数の型を持つことができ、その型は大抵クラスに由来します。ですから、あるオブジェクトの type(s)（型）を知ることで、オブジェクトがどのメッセージに応答するか推測できるようになります。

Ruby にはあらかじめ定義された class があります。すぐに思い浮かぶには手続き型言語のデータ型とオーバラップするクラス群でしょう。例えば、*String* クラスは文字列を定義します。*Fixnum* クラスは整数（integers）を定義します。あなたがプログラミング言語が提供すると想定しているデータ型について、Ruby はあらかじめクラスを定義しています。ですが、オブジェクト指向言語はそれ自体がオブジェクトから構築されています。そしてここからが話が面白くなっていくのです。

*String* クラスは、新たに作られる文字列オブジェクトのための設計図ですが、この String クラス自身もひとつのオブジェクトです。String クラスは `Class` クラスのインスタンスなのです。それぞれの文字列オブジェクトが個々にデータの異なる *String* クラスのインスタンスであるように、それぞれのクラスオブジェクト（*String*、*Fixnum*、などなど）は個々にデータの異なる *Class* クラスのインスタンスです。*String* クラスが新しい文字列を生み出すように、*Class* クラスは新しいクラスを生み出すのです。

オブジェクト指向言語はこのようにオープンエンディド（open-ended：終点が定義されていない状態）です。オブジェクト指向言語は、あなたをあらかじめ組み込まれた型と定義されている操作の小さな集合に閉じ込めることはありません。あなたは自分自身でまったくあたらしい型を発明することができるのです。オブジェクト指向で作られたアプリケーションはそれぞれ、あなたの関心領域のための特注された唯一無二のプログラミング言語に成長していきます。

オブジェクト指向言語である Ruby が、最終的にあなたに喜びをもたらすのか、それとも苦痛を与えるかはデザイン（設計）の良し悪しにかかっており、まさにそれが本書の関心です。
